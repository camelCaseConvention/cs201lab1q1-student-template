...
public class NumbersArray {
    ...
    static int findMax(Integer[] nums) {
        int max = -1;
        for (int n : nums)
            max = Math.max(n, max);
        return max;
    ...
    static Integer[] findDuplicates(Integer[] iii) {
        Integer[] input = Arrays.copyOf(iii, iii.length);
        Arrays.sort(input);
    ...
    static Integer[] findUnique(Integer[] iii) {
        Integer[] input = Arrays.copyOf(iii, iii.length);
        Arrays.sort(input);
        ArrayList<Integer> ans = new ArrayList<>();
        for (int i = 0; i < input.length; i++) {
